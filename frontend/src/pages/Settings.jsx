...
import { ShieldCheck, AlertCircle, Heart, Clock, RotateCcw, Save } from "lucide-react";
    ...
    return (
        <div className="space-y-5" data-testid="settings-page">
            <div className="flex items-center justify-between">
                <h1 className="font-heading text-3xl font-black tracking-tight">Alert SLA &amp; Escalation</h1>
            ...
            <Card className="border-slate-200 bg-amber-50/30">
                <CardContent className="p-4 text-sm text-slate-700">
                    <div className="font-bold text-slate-900 mb-1">How escalation works</div>
                    <ul className="list-disc pl-6 space-y-1 text-xs text-slate-600">
                        <li>The scheduler runs every <code>scheduler_interval_minutes</code> minutes.</li>
                        <li>On each cycle it checks every open alert against its SLA. If the age exceeds the threshold, the alert is escalated to the configured role (e.g. <code>supply_officer</code>, <code>procurement</code>, <code>hospital_manager</code>).</li>
                        <li>Each escalation appends an entry to the alert&apos;s history (visible on the Alerts page) — so you can audit who was paged and when.</li>
                        <li>Stale stock entries (no update for <code>no_update_minutes</code>) automatically generate a data-quality alert.</li>
                        <li>Changes take effect at the start of the next scheduler cycle.</li>
                    </ul>
                </CardContent>
            </Card>
        </div>
