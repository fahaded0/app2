...
usage() {
  cat <<'EOF'
Usage:
  deploy.sh --env-file PATH --release TAG_OR_FULL_SHA --base-url URL
            [--compose-file PATH] [--ca-certificate FILE | --insecure]
            [--timeout-seconds N] [--wait-seconds N]
            [--service-env-file PATH] [--skip-fetch]

Deploys the application from an explicit Git tag or full 40-character commit SHA.
...
safe_image_tag() {
  local ref="$1" commit="$2" tag
  tag="$(printf '%s' "$ref" | tr -c 'A-Za-z0-9_.-' '-' | cut -c1-64)"
  tag="${tag:-${commit:0:12}}"
  printf '%s\n' "$tag"
}

ensure_clean_tracked_tree() {
  git -C "$repo_root" diff --quiet || fail "Tracked working tree changes exist"
  git -C "$repo_root" diff --cached --quiet || fail "Staged changes exist"
}
...
health_args() {
  ...
  if [[ -n "$ca_certificate" ]]; then
    printf '%s\0' "--ca-certificate" "$ca_certificate"
...
write_state_file() {
  ...
  chmod 0700 "$(dirname "$path")" 2>/dev/null || true
  cat > "$path" <<EOF
...
release_commit="$(resolve_release "$release_ref")"
image_tag="$(safe_image_tag "$release_ref" "$release_commit")"

git -C "$repo_root" checkout --detach "$release_commit"
[[ -x "${repo_root}/deploy/scripts/health-check.sh" ]] || fail "deploy/scripts/health-check.sh is missing or not executable in release $release_commit"

compose=(docker compose --env-file "$env_file" -f "$compose_file")
APP_IMAGE_TAG="$image_tag" "${compose[@]}" config >/dev/null
APP_IMAGE_TAG="$image_tag" "${compose[@]}" build backend frontend
APP_IMAGE_TAG="$image_tag" "${compose[@]}" up -d --remove-orphans

if ! wait_for_health; then
  fail "Deployment health gate failed for release $release_ref ($release_commit)"
fi

write_state_file "${state_dir}/current-release.env" "$release_ref" "$release_commit" "$image_tag"
write_service_env_file "$service_env_file" "$release_ref" "$release_commit" "$image_tag"
